...
# --- Paths and options ---
# Verify that the following paths are correct.
...
# Use an absolute path, such as "/path/to/build_outputs", when build_outputs is elsewhere.
base_root_folder = os.path.abspath("/home/lhw/codetran/1/build_outputs") 

# Set to None to scan and decompile every optimization-level subdirectory.
target_opt_level = "O1" 
...
for dirpath, dirnames, filenames in os.walk(base_root_folder):
    for filename in filenames:
        if filename.endswith(".o"):
            ...
            # Get the file's parent path relative to the base root, for example 'O1' or 'O2/subfolder'.
            rel_path = os.path.relpath(dirpath, base_root_folder)
            
            # Use the first path component as the optimization-level label, such as O1, O2, or O3.
            ...
            # Enforce the requested optimization level unless target_opt_level is None.
            if target_opt_level and opt_level != target_opt_level:
                continue
                
            # Build and create the matching output directory, for example dec-human/O1.
